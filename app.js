...
const GROUPS = {
  A: {
    ...
    questions: [
      ...
      { text: "エレベーターの階数表示が特定の数字を通過するとき、一瞬だけ数え直してしまう", yes: { suuhi: 4 } },
      { text: "重要な決断ほど、コインを投げて出た面に従いたくなる瞬間がある", yes: { eki: 4 } },
      ...
      { text: "パスワードを決めるとき、語呂より「バランスの良い数字の並び」を優先する", yes: { suuhi: 4 } },
      { text: "「はい」か「いいえ」しか選べない世界の方が、実は生きやすいと思う", yes: { eki: 4 } },
      { text: "子どもの頃、星座早見盤や暦をいじって遊んだ記憶がある", yes: { astro: 2 } },
      { text: "自分の性格は「火」や「水」のような元素に例えられる方がしっくりくる", yes: { meijutsu: 2 } },
      { text: "迷ったときは、直近の出来事より「何年かけて繰り返してきたか」を思い出す方だ", yes: { astro: 2 }, no: { eki: 3 } },
      { text: "サイコロを転がすと、出た目に意味を探してしまう方だ", yes: { eki: 4 } },
      { text: "運命というものが実在するなら、それは変えられない方がロマンチックだと思う", yes: { meijutsu: 2 } },
      { text: "静寂しかない世界と雑音しかない世界、選べるなら静寂を選ぶ", yes: { suuhi: 1 }, no: { eki: 3 } },
      { text: "誰も見ていない鏡の中の自分は、今の自分と少し違う気がする", yes: { astro: 1 }, no: { meijutsu: 1 } },
      { text: "結果を人に伝えるとき、「巡り合わせ」という言葉の方が「積み上げ」という言葉よりしっくりくる", yes: { astro: 1 }, no: { suuhi: 2 } },
    ],
  },
  B: {
    types: ["consultant", "seimei", "zenze"],
    questions: [
      { text: "誰かの相談を受けたら、まず「今すぐ変えられること」を3つ挙げたくなる", yes: { consultant: 2 } },
      { text: "人の名前を聞くと、その字面や響きの意味をつい考えてしまう", yes: { seimei: 3 } },
      { text: "「前にもこんな感覚になったことがある」と、初めての場所で感じることがある", yes: { zenze: 3 } },
      { text: "相談された悩みは、感情よりまず構造(原因と結果)に分解したくなる", yes: { consultant: 1, seimei: 1 } },
      { text: "名刺交換をするとき、肩書きより名前の漢字の方が気になる", yes: { seimei: 3 } },
      { text: "強い既視感(デジャブ)を、ただの偶然だと片付けられない方だ", yes: { zenze: 3 } },
      { text: "誰かに改名や芸名を勧めるとしたら、運気が変わると本気で思っている", yes: { seimei: 2 } },
      { text: "相談相手には、答えより「次にやることリスト」を渡したくなる", yes: { consultant: 2 } },
  ...
  C: {
    ...
    questions: [
      { text: "絵や写真を見ると、そこに写っていないはずの「続きの物語」を想像してしまう", yes: { tarot: 3 }, no: { pendulum: 2 } },
      { text: "一つの記号やマークを見ただけで、パッと答えが浮かぶことがある", yes: { rune: 4 } },
      ...
      { text: "昨夜見た夢を、朝のうちにメモしたくなることがある", yes: { yume: 2 } },
      { text: "「はい/いいえ」で聞かれたら、体が先に傾く感覚がある方だと思う", yes: { pendulum: 4 } },
  ...
  D: {
    ...
    questions: [
      ...
      { text: "会う前から、その人の雰囲気を何となく感じ取れることがある", yes: { psychic: 4 } },
      { text: "誰かの悩みを聞くと、解決策より先に「大変だったね」と言いたくなる", yes: { healer: 2 } },
      { text: "誰かの言葉を伝えているとき、それが自分の考えなのか分からなくなる瞬間がある", yes: { channel: 4 } },
      { text: "初対面の人の靴や持ち物から、性格を勝手に想像してしまう", yes: { kansou: 3 } },
      { text: "理由が説明できないのに、ある人や場所を「苦手」と感じることがある", yes: { psychic: 2, channel: 2 } },
      { text: "友人が落ち込んでいたら、アドバイスよりそばにいることを選ぶ", yes: { healer: 2 } },
      { text: "一人で静かにしていると、誰かに話しかけられているような感覚になることがある", yes: { channel: 4 } },
      ...
      { text: "眠りに落ちる直前、知らない声や言葉が浮かんでくることがある", yes: { channel: 3 } },
      { text: "顔立ちや姿勢を見ただけで、その人の調子の良し悪しが分かる方だ", yes: { kansou: 2 } },
      { text: "宇宙人に出会ったら、質問するより先に握手を求めると思う", yes: { psychic: 2, channel: 2 } },
